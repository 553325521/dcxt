        ...
        interval = setInterval(function() {
            if ( (new Date().getTime() - start < maxtimeOutMillis) && !condition ) {
                ...
                if(!condition) {
                    ...
                    loggingService.info("'waitFor()' timeout");
                    phantom.exit(1);
                } else {
                    // Condition fulfilled (timeout and/or condition is 'true')
                    //loggingService.info("'waitFor()' finished in " + (new Date().getTime() - start) + "ms.");
...
if (system.args.length !== 2) {
    loggingService.info('Usage: run-qunit.js URL');
    phantom.exit(1);
}
...
// Route "loggingService.info()" calls from within the Page context to the main Phantom context (i.e. current "this")
...
page.onError = function (msg, trace) {
    ...
    trace.forEach(function(item) {
        loggingService.info('  ', item.file, ':', item.line);
    })
}
...
page.open(openPath, function(status){
    if (status !== "success") {
        loggingService.info("Unable to access network");
        phantom.exit(1);
    } else {
        ...
        waitFor(function(){
            ...
            if (coverageInfo.key){
                ...
                loggingService.info('Coverage for ' + coverageInfo.key + ' in coverage.html');
            }
            ...
            var failedNum = page.evaluate(function(){
                var el = document.getElementById('qunit-testresult');
                loggingService.info(el.innerText);
                try {
