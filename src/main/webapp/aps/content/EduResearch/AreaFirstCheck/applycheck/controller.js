        ...
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	var findExpertList=function(){
            		$httpService.post(config.findExpertListURL,{}).success(function(data) {
            			$scope.expertList=data.data;
                		$scope.$apply();	
    	            });
            	}
                ...
                $scope.success=function(){
                	$scope.form.AREA_CHECK_RESULT='1';
            		$scope.check();
                }
              //不通过
                $scope.fail=function(){
                	$scope.form.AREA_CHECK_RESULT='2';
                	var checkInfo = $scope.form.AREA_CHECK_OPINION;
    		  ...
    		  /* $scope.addExpert=function(){
	               	...
	               	if(values.length < 1){
	               		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"请选择您要删除的记录。"});
	               	}else{
	               		for(var i=0;i<values.length;i++){
	               			BUTTON_PK=values[i];
	               			$httpService.post(config.deleteURL,{"BUTTON_PK":BUTTON_PK}).success(function(data) {
	               					if(data.code != '0000'){
	                           		loggingService.info(data.msg);
	               					}	
	                           }).error(function(data) {
	                               loggingService.info('删除失败！');
	                           });
	               		}
