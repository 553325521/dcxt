...
public class TablesController extends BaseController {
	...
	public void removeTablesById(HttpServletRequest request, HttpSession seesion) {
		...
		try {
			Map<String, Object> map = getParameterMap();
			/*
			 * //先查询当前桌位是否在当前商铺，不然不能删除 String token =
			 * CookieUtils.getCookieValue(request, "DCXT_TOKEN"); String
			 * userJson = jedisClient.get(RedisConstants.REDIS_USER_SESSION_KEY
			 * + token); JSONObject userObj = JSONObject.parseObject(userJson);
			 * 
	...
	public void loadTableList(HttpServletRequest request, HttpSession seesion) {
		...
		try {
			Map<String, Object> map = getParameterMap();
			// 先查询当前桌位的排序序号
			map.put("sqlMapId", "loadTableList");
